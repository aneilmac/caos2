...
use crate::{ast::Command, CaosError, Rule, parser::parse_condition};
pub(crate) use command_parser_trait::CommandParser;
pub(crate) use command_thunk::CommandThunk;
use pest::iterators::{Pair, Pairs};

use super::{parse_expression, base::parse_label};
...
fn find_command_match<'i>(pair: Pair<'i, Rule>, remainder: &mut Pairs<'i, Rule>) -> Result<CommandThunk<'i>, CaosError> {
    let rule = pair.as_rule();
    match rule {
        Rule::command_dbg_asrt => {
            let condition = parse_condition(remainder)?;
            Some(CommandThunk::Completed(pair.clone(), Command::DbgAsrt { condition }))
        }
        Rule::command_gsub => {
            let label_p = remainder.next().ok_or_else(|| CaosError::new_end_of_stream())?;
            let destination = parse_label(label_p)?;
            Some(CommandThunk::Completed(pair.clone(), Command::Gsub { destination }))
        }
        Rule::command_goto => {
            let label_p = remainder.next().ok_or_else(|| CaosError::new_end_of_stream())?;
            let destination = parse_label(label_p)?;
            Some(CommandThunk::Completed(pair.clone(), Command::Goto { destination }))
        }
        _ => None 
    }
